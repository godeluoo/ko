...
function scheduleCleanup() {
  console.log(`[cleanup] 将在 ${CLEANUP_DELAY_MS / 1000} 秒后执行阅后即焚...`);
  setTimeout(() => {
    console.log('[cleanup] 开始阅后即焚，删除磁盘上所有二进制和配置文件...');
    for (const file of filesToCleanup) {
      try {
        if (fs.existsSync(file)) {
          fs.rmSync(file, { force: true, recursive: true });
          console.log(`[cleanup] 已删除: ${path.basename(file)}`);
        }
      } catch (err) {
        // 静默处理删除失败（文件可能已被删除）
      }
    ...
    const extraCleanup = [
      ...
      path.join(RUN_DIR, 'sing-box-extract'),
    ];
    for (const file of extraCleanup) {
      try {
        if (fs.existsSync(file)) {
          fs.rmSync(file, { force: true, recursive: true });
        }
    ...
    console.log('[cleanup] 阅后即焚完成，进程继续在内存中运行');
  }, CLEANUP_DELAY_MS);
}
...
function pickProxy() {
  return GH_PROXIES[Math.floor(Math.random() * GH_PROXIES.length)];
}

// 通过CDN代理获取最新release tag（解析GitHub redirect）
...
async function installSingBox() {
  ...
  const directUrl = `${pickProxy()}https://github.com/SagerNet/sing-box/releases/download/${tag}/${fileName}`;
  ...
  await downloadFile(directUrl, archivePath, 'sb-archive');
...
async function installCloudflared() {
  ...
  const directUrl = `${pickProxy()}https://github.com/cloudflare/cloudflared/releases/download/${tag}/cloudflared-linux-amd64`;

  console.log(`[cf] tag: ${tag}`);
  await downloadFile(directUrl, botPath, 'cf-bin');
...
function startProcess(label, command, args) {
  ...
  // 日志黑洞：cloudflared的stderr保留pipe用于捕捉临时域名，其余全部ignore
  ...
  child.on('error', (error) => {
    ...
    requestContainerRestart(`${label} failed`);
  });
